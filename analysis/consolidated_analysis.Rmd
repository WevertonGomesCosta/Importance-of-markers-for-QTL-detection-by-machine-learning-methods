...
# With the 'complete' step above, markers with 0 importance stay at 0 (min), while max scales to 10.
ml_data <- ml_data %>%
...
if (file.exists(gwas_file)) {
  ...
  # 3. Cleaning, Selection, and Filtering
  gwas_data <- df_raw %>%
    rename(
    ...
    # --- MODEL FILTERING AND RENAMING ---
    # 1. Remove NYC versions (Keep Kansas and SUPER)
    filter(!str_detect(method, "NYC")) %>%
    # 2. Clean names: Remove ".Kansas" suffix so they appear just as "Blink" or "FarmCPU"
    mutate(method = str_replace_all(method, "\\.Kansas", "")) %>%
    # ------------------------------------
  ...
  # Check which models remained
  cat("Models included:", unique(gwas_data$method), "\n")
...
```{r filter_variables_all, include=FALSE}
all_data <- all_data %>% 
  filter(variable %in% c(7:11, 13:17))
```
...
write.csv(coinc_plot, "output/Consolidated_Performance_Metrics.csv", row.names = FALSE)
...
```{r check_labels, include=FALSE, eval=FALSE}
coinc_plot %>% 
  filter(methodology == "ML") %>%
  select(variable, method, ngenes, herd, label_dp) %>% 
  as.data.frame()

coinc_plot %>% 
  filter(methodology == "MM") %>%
  select(variable, method, ngenes, herd, label_dp) %>% 
  as.data.frame()

coinc_plot %>% 
  filter(methodology == "ML") %>%
  select(variable, method, ngenes, herd, label_prec) %>% 
  as.data.frame()
...
coinc_plot %>% 
  filter(methodology == "ML") %>%
  select(variable, method, ngenes, herd, label_fp) %>% 
  as.data.frame()
...
coinc_plot %>% 
  filter(methodology == "ML") %>%
  select(variable, method, ngenes, herd, label_spec) %>% 
  as.data.frame()
...
Below, we present the mean and standard deviation for:
* **Hits (TP):** The number of unique true QTLs found.
* **Errors (FP):** The number of noise markers incorrectly selected.
...
write.csv(table_display, "output/Hits_Errors_Detailed_Table.csv", row.names = FALSE)
```

# 9\. Computational Efficiency (Runtime)
...
```{r runtime_analysis, fig.width=12, fig.height=6}
...
```{r runtime_analysis2, fig.width=12, fig.height=6}
if(nrow(time_data) > 0) {
  ...
  time_summary <- time_clean %>%
    ...
    select(method, type, Time_Format, Min_Sec, Max_Sec)
  
  write.csv(time_summary, "output/Runtime_Summary_Table_mean.csv", row.names = FALSE)
...
1.  **High Efficiency (MM):** Methods such as **BLINK** and **FarmCPU** (grouped under MM) generally exhibit the lowest execution times due to linear algebra optimizations, processing thousands of markers in seconds.
2.  **Cost of Learning (ML):** ML methods, especially **Random Forest** and **MARS** (particularly when fitting high-degree interactions), tend to be orders of magnitude slower, reflecting the exhaustive search for non-linear patterns and interactions.
3.  **Practical Implication:** For datasets with millions of SNPs, using ML may require marker pre-selection (screening) or High-Performance Computing (HPC) infrastructure, whereas MM scales more easily on personal computers.
